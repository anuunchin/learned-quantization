...
def my_custom_gradient(parameter, scale, penalty_threshold):
    ...
    inputs_quantized_nonrounded = (
        parameter / scale
    )  # changes in the scale affect the output inversely
    ...
    def custom_grad(dy, variables=None):        
        ...
        # reduce to mean according to shape
        if len(scale.shape) == 1:
            ...
            scale_grads_final = tf.where(
                all_above_threshold_exp,
                scale_grads_reduced * maxvalue,
                scale_grads_reduced * maxvalue
            )
            ...
            all_above_threshold_exp = tf.reshape(all_above_threshold, scale.shape)
            all_above_threshold_broad = tf.broadcast_to(all_above_threshold_exp, parameter.shape)
...
class CustomDenseLayer(tf.keras.layers.Layer):
    ...
    def __init__(
        ...
        seed: int, 
        orientation: Literal["rowwise", "columnwise", "scalar"] = "rowwise",
        name: str = "custom_dense_layer",
        ...
        **kwargs
        ...
        self.nested_q_b_layer = CustomQuantizedScaleLayer(
            penalty_threshold=penalty_threshold, initializer=None, orientation="scalar" #bias scaler is a scalar by default
        )
    ...
    def build(self, input_shape):
        self.W = self.add_weight(
            ...
            #    regularizer=tf.keras.regularizers.l2(self.l2_factor),   # Uncommenting this will affect quantization
            trainable=True,
        )
