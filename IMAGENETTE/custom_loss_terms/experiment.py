...
def initialize_model_with_trained_weights(
    ...
    x = custom_layers.CustomConv2DLayer(
        seed=seed,
        penalty_threshold=penalty_rate,
        orientation=orientation,
        initializer=initializer,
        filters=128,
        kernel_size=(3, 3),
...
def initialize_model(
    ...
    x = residual_block(x, 64, orientation=orientation, penalty_rate=penalty_rate, idx = 1, seed=seed)
    x = residual_block(x, 64, orientation=orientation, penalty_rate=penalty_rate, idx = 2, seed=seed)
    
    x = residual_block(x, 128, stride=(2, 2), orientation=orientation, penalty_rate=penalty_rate, idx = 1, seed=seed)
    x = residual_block(x, 128, orientation=orientation, penalty_rate=penalty_rate, idx = 2, seed=seed)
    
    x = residual_block(x, 256, stride=(2, 2), orientation=orientation, penalty_rate=penalty_rate, idx = 1, seed=seed)
    x = residual_block(x, 256, orientation=orientation, penalty_rate=penalty_rate, idx = 2, seed=seed)
    
    x = residual_block(x, 512, stride=(2, 2), orientation=orientation, penalty_rate=penalty_rate, idx = 1, seed=seed)
    x = residual_block(x, 512, orientation=orientation, penalty_rate=penalty_rate, idx = 2, seed=seed)
...
def initialize_loss_function(
    ...
    """
    layers = [layer for i, layer in enumerate(model.layers) if "custom_conv2d_layer" in layer.name]
