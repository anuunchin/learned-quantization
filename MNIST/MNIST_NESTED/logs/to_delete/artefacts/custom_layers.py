...
#        penalty = tf.where(max_bins <= 2.0, 0.0, penalty_rate * max_bins) 
        ...
        tf.print(tf.reduce_sum(scale_grads), output_stream=f'file://logs/temp/scale_grads.log')
...
#            scale_grads = tf.where(maxvalue <= 1.0, tf.abs(scale_grads), scale_grads)
            ...
            tf.print(proportion, output_stream=f'file://logs/temp/scale_grads.log')
            tf.print(maxvalue, output_stream=f'file://logs/temp/maxvalue.log')
...
class CustomQuantizedScaleLayer(tf.keras.layers.Layer):
    ...
    def __init__(self, penalty_rate, initializer, orientation):
        ...
        setup_logger("logs/temp", ["call_values.log", "scale_grads.log", "maxvalue.log", "counts_in_bias.log"])
