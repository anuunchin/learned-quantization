...
def my_custom_gradient(parameter, scale, penalty_rate):
    ...
    def custom_grad(dy, variables=None):
        if len(parameter.shape) == 1:
            ...
            # first apply negative gradient to non-sensitive rows
            scale_grads = tf.where(dy <= eps_float32, tf.abs(inputs_quantized_rounded), 0.0)
            scale_grads = (-1.0) * tf.expand_dims(tf.reduce_mean(scale_grads, axis=0), axis=0)
            # then apply positive gradient to sensitive rows with small bins

        else:
            maxvalue = tf.expand_dims(tf.reduce_max(tf.abs(tf.floor(parameter / scale)), axis=1), axis=1)
            scale_grads = tf.where(dy <= eps_float32, tf.abs(inputs_quantized_rounded), 0.0)
            scale_grads = (-1.0) * tf.expand_dims(tf.reduce_mean(scale_grads, axis=1), axis=1)

        scale_grads *= penalty_rate
