...
@tf.custom_gradient
def my_custom_gradient(parameter, scale):
    """
    Custom gradient function that simply enables STE.
    We do not provide gradients for scale factors here.
    """
...
class CustomQuantizedScaleLayer(tf.keras.layers.Layer):
    """
    Nested quantization layer that simply enables STE for parameter gradients.
    No gradients are provided to the scale factors.
    Checkout the my_custom_gradient function.
    """
    def __init__(self, penalty_rate, initializer, orientation):
        super(CustomQuantizedScaleLayer, self).__init__()
        self.initializer = initializer
        self.orientation = orientation
        self.penalty_rate_init = penalty_rate
        ...
        setup_logger(
            ...
            [
                ...
                "pos_grads.log",
                "zero_grads.log",
                "penalty_rate_tracker.log"
            ],
        )

    def build(self, input_shape):
        self.penalty_rate = self.add_weight(
            name="penalty_rate",
            shape=(),
            initializer=tf.keras.initializers.Constant(self.penalty_rate_init),
    ...
    def call(self, inputs):
        return my_custom_gradient(inputs, self.scale)
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.nested_q_w_layer = CustomQuantizedScaleLayer(
            penalty_rate=penalty_rate, initializer=None, orientation=orientation
        )
        self.nested_q_b_layer = CustomQuantizedScaleLayer(
            penalty_rate=penalty_rate, initializer=None, orientation="scalar" 
        )
