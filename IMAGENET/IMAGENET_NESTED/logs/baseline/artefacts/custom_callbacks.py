...
class QuantizedDataTrackingCallback(tf.keras.callbacks.Callback):
    def __init__(self, layer, log_dir):
        ...
        self.layer = layer
        self.log_file_path_kernel = f"{log_dir}/{layer.__class__.__name__}_scale_{layer.scale.shape}.log"

    def on_epoch_end(self, epoch, logs=None):
        kernel = self.layer.scale.numpy().flatten()
...
class NestedScaleTrackingCallback(tf.keras.callbacks.Callback):
    ...
    def on_train_end(self, logs=None):
        # Track floored quantized values in kernel at the end of training
        inputs_quantized_nonrounded = self.layer.kernel / self.k_scale
        ...
        # Track unique floored quantized values in kernel at the end of training
    ...
    def on_train_begin(self, logs=None):
        # Track floored quantized values in kernel at the start of training
        inputs_quantized_nonrounded = self.layer.kernel / self.k_scale
        ...
        # Track unique floored quantized values in kernel at the start of training
